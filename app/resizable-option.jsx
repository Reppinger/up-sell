class ResizableOption extends React.Component {
    constructor(props) {
        super(props);
    ...
    getStyle(width, multiplier) {
        let value = width * multiplier;
        return {
            top: value,
            bottom: value,
            left: value,
    ...
    handleResize(event) {
        let baseWidth = 1+Math.round(window.innerWidth / 300);
        this.setState({
            taggerStyle: {  top: this.props.order*baseWidth*10,
                            width:baseWidth*10,
                            height: baseWidth*10},
            textStyle: {left:baseWidth*13, fontSize:7*baseWidth}
        })
    }
    render() {
        return <div>
